# CoEditor · v0.6.0（M1 / M1.5 / M2 / M3 / M4 / M5 全部完结）

零依赖（只用 Node 内置模块 + 原生前端），画布式本地文档批注层：人在网页上写字，批注按原文位置落锚、永不删除，Agent 改文档时自动重定位，批注作为**约束**回流给 Agent。**M5 起：Markdown 阅读 ⇄ 编辑双态（VSCode 式双击进源码）、选区浮条（批注 / 高亮 / 删除线）、批次系统（每轮迭代一组批注）、白板、像 VSCode 一样打开本地文件夹。** 数据目录沿用 `.marginalia/`（历史约定，纯 JSON 可 git 管理）。
...
- **选区动作菜单（M5）**：选中文字即弹浮条——批注（写约束）/ 高亮（mark 批注，黄底）/ 删除线（mark 批注，删除线样式），高亮与删除线一键落锚无需写字
- **批次系统（M5）**：批注按批次（round）归属，「保存本批次」或 Agent 外部修改自动归档推进；约束输出按批次分组；批注历史抽屉分组展示
- 本地文件夹 → 网页文件树（md / txt / html / json / csv / **pdf** / **图片**），左栏一键切换 vault
