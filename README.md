# CoEditor · v0.8.0（三模式架构 · 稳定版）

**发布门禁：`node tools/run-battery.mjs` 一键 7/7 真实鼠标 E2E 全绿**（阅读/编辑/画布全交互，隔离 vault + 隔离端口）。详见 [CHANGELOG.md](CHANGELOG.md) · 开发者接手指南见 [docs/HANDOFF.md](docs/HANDOFF.md)。

**v0.7.0 是一次体验重构**：默认进入「阅读」——文档按宽度自适应、纵向自然滚动、批注卡在右侧同屏成列，不再把长文档缩成一张缩略图；「编辑」提供占满视口的 Markdown / HTML 源码编辑器（CodeMirror：语法高亮 / 行号 / 撤销 / ⌘F 搜索；HTML 编辑带实时预览分屏）；「画布」保留全部 Cowart 式视觉协作（手绘箭头、便签、贴图、区域框选）。HTML 在隔离 iframe 中按真实网页呈现（保留原 CSS / 布局 / 相对资源，剥离脚本），批注锚定进 iframe；**双击段落直接改文字**——原始 DOM 保真 + 路径映射回写，源文件的 URL / 脚本 / 结构原样保留，只有文字变更落盘。PDF 按 devicePixelRatio 做 HiDPI 渲染（DPR=2 屏幕上像素密度翻倍，不再发糊）；**长文档按需渲染**——文本层与批注锚定即时建立，页面画布滚近视口才绘制，几十页的 PDF 秒开不卡顿。侧栏 220–480px 可拖拽并记忆；顶栏精简为「阅读 / 编辑 / 画布 · 完成本轮批注 · 交给 Agent · 批注」，次要视图项收进 ••• 菜单；暖白纸感主题。批次语言重命名：保存本批次 → 完成本轮批注；复制 Agent 指令 → 交给 Agent（先展示约束清单再复制）；外部文件变化只重新锚定、**不再自动推进轮次**（是否交付由人决定）。
