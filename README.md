# CoEditor · v0.6.0（M1 / M1.5 / M2 / M3 / M4 / M5 / M6 全部完结）

零依赖核心（Node 内置模块 + 原生前端；PDF.js 与 mammoth 为 vendor 的渲染库），画布式本地文档批注层：人在网页上写字，批注按原文位置落锚、永不删除，Agent 改文档时自动重定位，批注作为**约束**回流给 Agent。**M5：Markdown 阅读 ⇄ 编辑双态、选区浮条（批注 / 高亮 / 删除线）、批次系统、白板。M6：PDF 连续阅读 + 列布局、Word (docx) 渲染批注、HTML 荧光标记、画布边界。** 数据目录沿用 `.marginalia/`（历史约定，纯 JSON 可 git 管理）。
...
- **选区动作菜单（M5/M6）**：选中文字即弹浮条——批注（写约束）/ 高亮（mark 批注，黄底）/ 删除线（mark 批注，删除线样式）。md / docx / html 全格式可用；HTML 上即荧光标记
- **批次系统（M5）**：批注按批次（round）归属，「保存本批次」或 Agent 外部修改自动归档推进；约束输出按批次分组；批注历史抽屉分组展示
- **PDF 连续阅读（M6）**：多页展开为连续长纸（页间纸缝），页宽自适应纸张**不再横向溢出**；顶栏 ▦1/▦2/▦3 切换单列 / 双列 / 三列阅读布局；文本层锚定跨页可用
