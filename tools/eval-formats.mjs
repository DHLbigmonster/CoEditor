...
result.html = await evaluate(`(() => {
  ...
  if (!frame || !frame.contentDocument) return { error: "no iframe" };
  const el = [...frame.contentDocument.querySelectorAll("p, li, h2, h3")].find((n) => n.textContent.trim().length > 10);
  if (!el) return { error: "html no text" };
  const r = el.getBoundingClientRect();
  const fr = frame.getBoundingClientRect();
  return { x1: r.left + fr.left + 4, y1: r.top + fr.top + r.height / 2, x2: r.right + fr.left - 50, y2: r.top + fr.top + r.height / 2, sample: el.textContent.trim().slice(0, 16) };
})()`);
if (result.html && !result.html.error) {
  ...
  result.htmlAfter = await evaluate(`(() => {
    const frame = document.getElementById("html-frame");
    const innerSel = frame && frame.contentWindow ? String(frame.contentWindow.getSelection()).trim().length : -1;
    return {
      selLen: innerSel,
      menuShown: !document.getElementById("sel-menu").hidden,
    };
