...
44. **右栏收敛（v1.5.2）**：#cards 结构 = 静态头部（cards-head 含 ✕）+ #cards-body（renderCards 容器）——innerHTML 只清 body 别抹头部。宽度 240–420 走 --cards-w，resizer 按工作区右边界算宽（main.getBoundingClientRect().right - clientX），拖过 200 阈值折叠。空侧栏默认关 + 用户主动打开标记 state.userOpenedCards。分组主 tabs 只留 待修改/已修改/已修改历史，保留与版本在 ⋯ 下拉。
45. **composer 自动保存**：新建批注 600ms 防抖自动创建（flushComposer），后续编辑 PATCH；blur 与 click 并发触发必须 in-flight 互斥（composerFlushInFlight）——否则双 POST 双创建。closeComposer 前 flush 不得丢草稿。
