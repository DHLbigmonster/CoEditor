...
import { offsetsOf, allocateNo, verifyRetainedText } from './review.mjs';
...
/* 登记前置：路径规范化、内容哈希、原样快照。docPath/filePath 必须是调用方 safeResolve 过的绝对路径。
   哈希与快照来自同一次读取的字节：登记中途文件被改，快照与记录也不会不一致。 */
export async function prepareVersionRegistration({ docPath, filePath, snapshotsDir }) {
  ...
  try {
    ...
    await Promise.all([
      writeFile(snapshot.source, sourceRaw),
      writeFile(snapshot.next, nextRaw),
    ]);
  } catch (error) {
...
export function registerVersion(data, { doc, file, note = '', id, sourceHash = '', nextHash = '', snapshot = null }) {
  ...
  if (existing) {
    // 同轮同文件再次登记：内容与来源都相同 = 幂等（已验收状态不得被重置回 pending）；
    // 新稿或原稿任一内容变化 = 对照基准变了，另立新条目
    const sameNext = !nextHash || !existing.nextHash || existing.nextHash === nextHash;
    const sameSource = !sourceHash || !existing.sourceHash || existing.sourceHash === sourceHash;
    if (sameNext && sameSource) {
...
/* 「保留的内容持续有效」：把源文档生效中的保留标记带到新版本文件。
   ...
   逐条用 verifyRetainedText 校验新稿（完整原文精确存在才算通过）：
   - intact    → 正常继承
   - missing   → 继承约束但标记「保留内容缺失，待确认」，绝不静默成功
   - ambiguous → 继承并标记「多处出现，待确认」
   - unverified（HTML 等）→ 继承并标记「未核验」，不假装校验过 */
export function carryRetainedAnnotations(data, { fromDoc, toDoc, toText, isHtml = false }) {
  ...
  for (const item of source) {
    ...
    const verdict = verifyRetainedText(toText, item.quote, item.prefix, { isHtml });
    const lost = verdict.state === 'missing';
    const ambiguous = verdict.state === 'ambiguous';
    const unverified = verdict.state === 'unverified';
    const bodyNote = lost
      ? `【保留内容缺失，待确认】原文在新版中未找到。原要求：${item.body || '（未填写）'}`
      : ambiguous ? `【保留内容多处出现，待确认】${item.body || '（承自上一版的保留要求）'}` : item.body;
    const entry = {
      id: 'A-' + randomUUID(),
      version: 1,
      ...
      status: 'active',
      weight: 1,
      history: [{ event: `carried_from_${item.id}`, at: new Date().toISOString(), ...(lost ? { note: '原文在新稿中未找到' } : ambiguous ? { note: '原文多处出现，无法确定所指' } : unverified ? { note: '标记文本无法校验正文' } : {}) }],
      supersedes: [],
      conflicts_with: [],
      carriedFrom: item.id,
      carriedFromDoc: fromDoc,
      ...(lost ? { anchorStatus: 'missing' } : ambiguous ? { anchorStatus: 'ambiguous' } : unverified ? { anchorStatus: 'unverified' } : {}),
      ...(verdict.offsets ? { offsets: verdict.offsets } : {}),
    };
    target.push(entry);
...
/* 段落级对照：保留顺序的 LCS——段落重排会如实显示为「删旧 + 增新」，
   而不是被多重集合比较抹成「毫无变化」。行内容不做 trim、空行保留：
   缩进和空白是 Markdown 语义的一部分，抹掉它们会把真实修改显示成「未变」。 */
