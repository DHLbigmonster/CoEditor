...
export async function preparePreview({ absPath, cacheRoot, force = false }) {
  ...
  const index = await readSlideIndex(absPath);
  const manifest = {
    status: "ready",
    ...
    fonts: await referencedFonts(absPath).catch(() => []),
...
async function loadFontIndex(cacheRoot) {
  ...
  try {
    ...
    if (Date.now() - Number(cached.builtAt || 0) < FONT_INDEX_TTL_MS && Array.isArray(cached.families)) return cached;
  } catch { /* 没缓存或缓存损坏，重建 */ }
...
export async function checkFonts(names, { cacheRoot, refresh = false } = {}) {
  ...
  try {
    ...
    const index = await loadFontIndex(cacheRoot);
