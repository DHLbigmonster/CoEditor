...
async function removeStaleLock(lockFile) {
  try {
    ...
    let alive = true;
    try { process.kill(info.pid, 0); } catch (error) { alive = error.code !== 'EPERM'; }
    if (alive && Date.now() - info.at < LOCK_TTL_MS) return false;
...
export async function acquireStoreLock(file) {
  ...
  return async () => {
    try {
      const info = JSON.parse(await readFile(lockFile, 'utf8'));
      if (info.token !== token) throw new Error('store-lock-owner-changed');
